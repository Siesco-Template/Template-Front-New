...
const S_Badge: FC<I_BadgeProps> = ({ status = 'default', variant = 'default', text, className, ...props }) => {
    const renderIcon: Record<I_BadgeVariant, () => ReactNode> = {
        default: () => <span className={styles.dot} />,
        primary: () => (
            <span className={styles.checkIcon}>
                <Tick width="10" height="10" color="#fff" />
            </span>
        ),
    };

    return (
        <span data-status={status} className={cls(styles.badge, styles[`variant-${variant}`], className)} {...props}>
            {renderIcon[variant]()}
            {text}
        </span>
    );
