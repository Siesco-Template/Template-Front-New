...
const ResetPassword = () => {
    ...
    async function handleFormSubmit(data: IInputs) {
        ...
        try {
            ...
            showToast({ label: error?.data?.message || 'Əməliyyat uğursuz oldu', type: 'error' });
        } finally {
            setLoading(false);
        }
    }
    ...
    return (
        <div className="w-full flex flex-col items-center gap-[32px] !mt-[80px]">
            <h3 className="text-center text-[#002C68] !text-[20px] font-medium">Şifrəni yenilə</h3>
            <p className="w-[74%] text-center text-[#05194AB3] leading-[21px]">
                Şifrəni dəyişmək istəyirsiniz? Zəhmət olmasa yeni şifrənizi aşağıda qeyd edin.
            </p>
